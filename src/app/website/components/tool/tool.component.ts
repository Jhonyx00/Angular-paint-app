...
export class ToolComponent implements AfterViewInit {
  ...
  initPosition() {
    const button = this.renderer.selectRootElement(
      `.btn-${this.toolGroupName}`,
      true
    );

    const { width, top } = button.getBoundingClientRect();

    console.log(button);

    const tool = this.renderer.selectRootElement('.tool-item-container', true);

    this.renderer.setStyle(tool, 'left', width + 'px');
    this.renderer.setStyle(tool, 'top', top + 'px');
  }
